...
В систему добавлены сервисы [minio и nginx](./docker-compose.yaml) (эмуляция CDN).

Сервис [reports](./reports/src/api/reports.py) изменен в соответствии с логикой задания.

[Конфиг Nginx](./cdn/nginx.conf).

Bucket в Minio сконфигурирован для анонимного доступа, однако в production-окружении не обходимо будет организовать ограничение доступа.
Доступ к web-интерфейсу Minio (порт 9001) намеренно открыт для проверки результатов и отладки.  
Структура хранения:
- Бакет (reports)
  - user_id1
    - report_date1_1
...
Инвалидация кэша в Nginx определяется настройкой TTL (proxy_cache_valid 200 302 1h;) в [конфиге](./cdn/nginx.conf) так как изменение отчетов после их создания не предполагается.

Проверка работы кэширования может быть осуществлена при помощи поля Created at в UI.

**Итог**: уменьшена нагрузка на OLAP DB за счет сохранения ранее сформированных отчетов в S3 и кэширования в CDN.

## Задание 4. Повышение оперативности и стабильности работы CRM.

Для снятия нагрузки с CRM DB был развернут CDC (Debezium + Kafka):
- Kafka запущена в режиме [KRaft](https://kafka.apache.org/41/operations/kraft/).
- Настройки Debezium в [манифесте оркестратора](./docker-compose.yaml) и [скрипте инициализации](./cdc/debezium_init.sh).
- Схема обработки данных в OLAP DB (ClickHouse) представлена в [файле разметки](./olap_db/init.sql).
  
Схема обработки данных:

- *_kafka_queue -  Data Stream Consumer для обработки сообщений Kafka;
- *_mv - MaterializedView, используемые в данном случае как парсеры Debezium JSON;
- *_replica - реплики исходных данных;
- *_mart - итоговая витрина для представления информации из CRM DB.

![](./arch/cdc_olap_db_er.png)

<details>
<summary>Использование MaterializedView лишь в качестве парсера</summary>
В <a href="https://clickhouse.com/docs/ru/materialized-view/incremental-materialized-view#background"> официальной документации </a> сказано: "Основной мотив использования Materialized Views состоит в том, что результаты, вставляемые в целевую таблицу, представляют собой итог агрегации, фильтрации или трансформации строк.". Данные пользователя из таблиц CRM DB не нуждаются в подобной обработке при записи в таблицы аналитической БД. В данной задаче использование MaterializedView хорошо подошло бы для статистической обработки телеметрических данных, но, согласно заданию, мы настраиваем CDC для CRM DB, а не TM DB.
</details></br>

**Итог**: уменьшена нагрузка на CRM DB за счет перенаправления аналитических запросов в OLAP DB с подготовленной витриной.
