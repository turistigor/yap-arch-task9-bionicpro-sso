...
Инвалидация кэша в Nginx определеяется настройкой TTL (proxy_cache_valid 200 302 1h;) в [конфиге](./cdn/nginx.conf) так как изменение отчетов после их создания не предполагается.

Проверка работы кэширования может быть осуществлена при поля Created at в UI.
